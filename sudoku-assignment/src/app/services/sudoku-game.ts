...
export class SudokuGame {
    ...
    //Check the row
    if (gameBoard[input.cellRow].some(c => c.value === input.newValue && c.column !== input.cellCol)){
      cell.invalid = true;
      isValid = false;
    } 
    //Check columns
    if (gameBoard.some(row => row.at(input.cellCol)?.value === input.newValue && row !== gameBoard[input.cellRow])){
      cell.invalid = true;
      isValid = false;
    } 
    // Mini-square
